...
결과: data/panel_amount.parquet (날짜×티커, 원)
...
COLS = {"거래대금": "amount"}   # 기존 패널에 없던 거래대금만 별도 수집
LOG = open(OUT / "panel_amount_log.txt", "a")
...
def upload():
    r = sh("gh", "release", "upload", "data-latest", *[str(p) for p in OUT.glob("panel_amount*")], "--clobber")
...
"""
    Path("PROGRESS_amount.md").write_text(md)
    for _ in range(3):
        sh("git", "add", "PROGRESS_amount.md")
...
try:
    ...
    save(buf); upload()
    done_n = len(buf["amount"]) - prev
    write_progress(max(done_n, 0), n, prev, (time.time() - t0) / max(done_n, 1),
                   str(max(buf["amount"]).date()) if buf["amount"] else "-", empty, "DONE")
